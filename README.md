...
**절대 하지 않는 것:** 매수, 매도, 주문, 잔고 조회. 이 서버는 어떤 경우에도 사용자 자금이 움직이는 동작을 수행하지 않는다. 한국투자증권 OpenAPI는 read-only 권한 신청 자체가 불가능하므로(키 권한으로는 막을 수 없음), 본 MCP는 **코드에 매매 엔드포인트를 처음부터 정의하지 않는 것**을 유일 방어선으로 삼는다. 자세한 내용은 [SECURITY.md](SECURITY.md)를 읽어라.
...
⚠️ **참고:** 한국투자증권 OpenAPI는 "조회 전용(read-only) 권한"이라는 발급 옵션을 제공하지 않는다. 즉 발급받는 키는 본질적으로 매매 권한을 포함한다. 그래서 본 MCP는 **코드 자체에 매매 엔드포인트를 한 줄도 작성하지 않는 방식**으로 매매를 차단한다. 키를 별도로 분리해서 보관하고, 본 MCP 외부에는 노출하지 않는 것을 권장한다.
