...
- OHLCV 시계열 → get_chart (장기간은 period=week/month로 다운샘플 손실 줄이기)
...
- get_chart: 최대 500포인트 (초과 시 균등 다운샘플)
...
function registerTools(server: McpServer, client: KisClient, kv: KVNamespace): void {
  ...
  server.tool(
    "get_chart",
    [
      "OHLCV 시계열을 day/week/month/year 단위로 조회.",
      "500포인트 초과 시 균등 다운샘플 (downsampledTo 필드 표시).",
      "장기 분석은 period=week/month로 늘려 다운샘플 손실 줄이기.",
      "기본 startDate는 period에 따라 자동 (day=6M, week=2Y, month=5Y, year=20Y).",
    ].join(" "),
    {
      symbol: SYMBOL_SCHEMA.describe("종목코드"),
      period: z.enum(["day", "week", "month", "year"]).describe("봉 단위"),
      startDate: z
        .string()
        .regex(/^\d{8}$/)
        .optional()
      ...
      adjusted: z.boolean().optional().describe("수정주가 여부 (기본 true)"),
      maxPoints: z
        .number()
        .int()
        .min(1)
        .max(500)
        .optional()
        .describe("최대 반환 포인트 수 (기본 500, 상한 500)"),
    },
    async (args) => {
      try {
