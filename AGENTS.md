...
본 문서는 한국투자증권 OpenAPI를 기반으로 하는 **조회 전용 MCP 서버**를 Cloudflare Workers에 배포하기 위한 요구사항을 정의한다. Codex는 이 문서를 기준으로 TypeScript 프로젝트를 처음부터 작성한다.
...
### 1.1. 목적
Codex에게 한국 주식/ETF 시장 데이터 분석 능력을 부여한다. 시세 조회, 수익률 분석, ETF 구성 분석, 신용거래율 확인, 고급 검색 등의 기능을 제공하되, 매매 실행은 원천적으로 차단한다.
...
- Codex Desktop 또는 MCP Inspector로 연결하여 각 도구 수동 호출
- 예상 쿼리:
...
### 9.2. 배포 후
- `wrangler deploy` 후 Codex Desktop의 `mcp-remote`로 워커 URL 연결
- 동일한 쿼리들로 프로덕션 검증
...
> 참고: 과거 ROADMAP.md(M0~M8)는 모두 구현 완료되어 본 §11 단계 위에 적층되어 있음 (DRY/병렬화 유틸, enrichment, 해외 주식 도구, 응답 캐싱, scheduled handler, Bearer 인증, 추가 Prompt 등). 향후 개선은 ROADMAP.md를 갱신하여 명시.
...
문서만 변경(README/SECURITY/TOOLS/Codex 등)한 경우 워커 배포는 생략 가능하지만 GitHub push는 반드시 수행한다.

---

## 13. MCP 출력 스키마 규칙

- 모든 MCP tool은 `tools/list` 응답에서 `outputSchema`가 노출되어야 한다.
- 성공 응답은 `structuredContent`를 반환해야 하며, 기존 MCP 클라이언트 호환을 위해 같은 데이터를 `content`의 JSON text로도 제공한다.
- 출력값이 JSON object가 아닌 경우 `{ "value": ... }` 형태로 감싸 object schema를 만족시킨다.
- 새 tool 추가 또는 tool 등록부 변경 시 출력 스키마 누락 경고가 다시 뜨지 않도록 `registerTools`의 공통 output schema 적용 경로를 유지하거나 명시적 output schema를 선언한다.
- 코드 변경 후 `npm run typecheck`와 관련 테스트로 output schema/structuredContent 경로가 깨지지 않았는지 확인한다.
